...
#elif TWK_BUNDLE_SOURCE_DEFLATED
#include "bundle_z.h" // generated by js/build.mjs, deflated
#else
...
#if (TWK_BUNDLE_BYTECODE && TWK_BUNDLE_BC_DEFLATED) || (!TWK_BUNDLE_BYTECODE && TWK_BUNDLE_B64_DEFLATED)
#define TWK_BUNDLE_NEEDS_INFLATE 1
#include <zlib.h>
#else
#define TWK_BUNDLE_NEEDS_INFLATE 0
#endif
...
#else
    // Source form, and the default: minified JS deflates to ~338KB where the
    // equivalent bytecode costs ~545KB, and the download is paid for hundreds of
    // millions of times while the extra parse is paid once per app lifecycle.
    // (Also the only form available when cross-compiling without a host
    // twk-bundlec.) Configure with -DTWK_BUNDLE_SOURCE=OFF to embed bytecode.
    std::vector<uint8_t> bytes = base64::decode(twk_bundle_b64, twk_bundle_b64_len);
#if TWK_BUNDLE_B64_DEFLATED
    if (!inflate_bundle(bytes, twk_bundle_b64_raw_len, error)) {
        return false;
    }
#endif
