...
The built library *contains* the JS bundle and the QuickJS engine, so anyone
redistributing a binary redistributes those works too; their notices are in
[THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md), regenerated by
`node scripts/generate-notices.mjs` from whatever the bundle actually pulled in.
All of it is permissive (MIT, Apache-2.0, BSD, BlueOak, Unlicense).
